...
USER_PAGES = CONFIG.get("user_pages", {})
PIPELINE = ROOT / "xpl" / "render-user-page.xpl"
STYLESHEET = ROOT / "xsl" / "wikipedia-userpage.xsl"
...
def render_page(xml_source: Path, wikitext_output: Path) -> None:
...
def main() -> int:
    for _, page in USER_PAGES.items():
        render_page(ROOT / page["xml_source"], ROOT / page["wikitext_output"])
    return 0
